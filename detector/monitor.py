...
def parse_log_line(line: str) -> dict | None:
    ...
    Returns a dict with normalized fields, or None if line is invalid.
    """
    line = line.strip()
    if not line:
        return None
    ...
    try:
        ...
        return {
            # Take first IP if X-Forwarded-For contains a chain e.g "1.2.3.4, 10.0.0.1"
            ...
            # parsed_at = when WE processed it, used for sliding window timing
            "parsed_at":     time.time(),
        }
...
def tail_log(log_path: str, callback):
    ...
    - Seeks to END on startup (ignores old historical lines)
    - Detects log rotation (file shrinks) and reopens
    - Calls callback(entry) for every valid parsed line
    ...
    with open(log_path, "r") as f:
        # Seek to end — ignore historical lines on startup
        f.seek(0, 2)
        last_size = f.tell()
        ...
        while True:
            ...
            if line:
                ...
                # Skip entries with no IP (internal health checks etc.)
                ...
                try:
                    ...
                    if current_size < last_size:
                        # File was rotated or truncated — reopen from start
                ...
                time.sleep(0.1)  # 100ms poll — low CPU, fast enough
